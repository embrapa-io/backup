...
# (evita acúmulo de diretórios órfãos io_matomo_* quando o backup aborta).
...
# io_matomo_* órfãos (de execuções que abortaram antes de compactar).
# A data é lida do nome do item: io_matomo_AAAA-MM-DD_HH-MM-SS[.tar.gz]
...
for ENTRY in "$BKP_PATH"/io_matomo_*; do
  [ -e "$ENTRY" ] || continue
  NAME=$(basename "$ENTRY")

  # extrai AAAA-MM-DD do nome; nomes fora do padrão são preservados
  FDATE=$(echo "$NAME" | sed -n 's/^io_matomo_\([0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}\)_.*/\1/p')
  if [ -z "$FDATE" ]; then
    continue
  fi
